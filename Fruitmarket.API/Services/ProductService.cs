...
namespace FruitMarket.API.Services
{
    public class ProductService : IProductService
    {
        ...
        public ProductService(IProductRepository productRepository, IBracketRepository categoryRepository, IUnitOfWork unitOfWork, IMemoryCache cache)
        {
        ...
        {
            // Here I list the query result from cache if they exist, but now the data can vary according to the Bracket ID, page and amount of
        ...
        public async Task<ProductResponse> SaveAsync(Product product)
        {
            ...
            {
                /*
                 Notice here we have to check if the Bracket ID is valid before adding the product, to avoid errors.
                 You can create a method into the CategoryService class to return the Bracket and inject the service here if you prefer, but 
                 it doesn't matter given the API scope.
                */
                var existingCategory = await _categoryRepository.FindByIdAsync(product.CategoryId);
                if (existingCategory == null)
                    return new ProductResponse("Invalid Bracket.");
